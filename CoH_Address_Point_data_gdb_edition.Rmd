...
Data is State Plane, Texas South Central Zone 5401, FIPS 4204, NAD83, EPSG 2278
...
roadtypes %>% filter(!is.na(roadtype)) %>% 
...
#   turn NA prefixes and roadtypes into blanks

dfnew <- dfnew %>% 
  mutate(prefix=stringr::str_trim(prefix)) %>% 
  mutate(prefix=replace_na(prefix, "")) %>%  # turn NA into empty for consistency
  mutate(roadtype=replace_na(roadtype, "")) # turn NA into empty for consistency

#   Let's shrink dataset size
dftest <- 
dfnew %>% sf::st_drop_geometry() %>% # This will speed things considerably
  select(siteaddid, addrnum, suffix, prefix, New_street, 
         roadtype, zipcode, xcoord, ycoord) 

#   Now start analysis by street name, type, and zipcode
#   For each name, type, zip group sum prefixes and NA's

tmp <- 
dftest %>% group_by(zipcode, New_street, roadtype) %>% 
      summarise(n=n(),
                sumNA = sum(prefix==""),
  ...
  mutate(singlet = n-sumNA) %>% # how many labelled prefixes are there?
  filter(!sumNA==n) %>% # drop any where none have a prefix
  filter(sumNA>0) %>% # drop any where all have a prefix
  mutate(percentNA = sumNA/n) # percent that are NA

#   Take singlet prefixes with total n>10 and find minimum distance to
#   a cousin

#   data frame of single occurrence prefixes
#     For potentially many records, only one has a prefix

df_singles <- tmp %>% 
  filter(n>10) %>% 
  filter(singlet==1) %>% 
  filter(!roadtype == "CIR" | is.na(roadtype)) %>% # eliminate circles
  filter(!roadtype == "CT" | is.na(roadtype)) %>%   # eliminate courts
  mutate(nearest=NA)

for (i in 1:nrow(df_singles)) {
  tmp_sing <- dftest %>% filter((zipcode==df_singles[i,]$zipcode) &
                                (New_street==df_singles[i,]$New_street) & 
                                (roadtype==df_singles[i,]$roadtype))
  
  # bad_X <- tmp_sing[!is.na(tmp_sing$prefix),]$xcoord
  # bad_Y <- tmp_sing[!is.na(tmp_sing$prefix),]$ycoord
  bad_X <- tmp_sing[(tmp_sing$prefix!=""),]$xcoord
  bad_Y <- tmp_sing[(tmp_sing$prefix!=""),]$ycoord
  
  tmp_sing <- tmp_sing %>% 
    mutate(dist=sqrt((xcoord - bad_X)^2 +
                     (ycoord-bad_Y)^2)) %>% 
    arrange(dist)
...
dfnew <- dfnew %>% mutate(old_prefix=prefix)
  
for (i in 1:nrow(df_singles)){
  dfnew[(dfnew$New_street %in% df_singles[i,]$New_street) &
        (!is.na(dfnew$prefix)) &
        (dfnew$zipcode %in% df_singles[i,]$zipcode) &
        (dfnew$roadtype %in% df_singles[i,]$roadtype),]$prefix <- ""
}

ObjID <- dfnew %>% filter(prefix=="", # who did we change?
                          old_prefix!="")   
...
df_cands <- tmp %>% 
  ...
  filter(!roadtype == "CIR") %>% # eliminate circles
  filter(!roadtype == "CT")  # eliminate courts
...
# for (i in 1:3){
  ...
  eraseme <- dftest %>% 
    filter((zipcode %in% df_cands[i,]$zipcode) &
           ...
           (roadtype %in% df_cands[i,]$roadtype))  
  
  targets <- eraseme %>% filter(prefix=="") %>% 
    mutate(prefix="",
           dist=0)
  
  for (j in 1:nrow(targets)){
    foo <-   eraseme %>% 
      #   Calculate all distances to target
    mutate(nearest=sqrt((xcoord -targets[j,]$xcoord)^2 +
                        (ycoord-targets[j,]$ycoord)^2),
              old_prefix=prefix,
              object=siteaddid) %>%
      #   sort shortest to top
      arrange(nearest) %>% 
      #   drop the targets
      filter(prefix!="")
    
    dftest[targets[j,]$siteaddid == dftest$siteaddid,]$prefix <- foo[1,]$old_prefix
    dftest[targets[j,]$siteaddid == dftest$siteaddid,]$Nearest <- foo[1,]$nearest
    dftest[targets[j,]$siteaddid == dftest$siteaddid,]$Neighbor_object <- foo[1,]$object
    #print("--b--")
  }
}
...
dfnew[dfnew$siteaddid %in% foo$siteaddid,]$prefix <- foo$prefix
...
dfnew$municipality %>% unique() %>% sort()
...
#   Oops, found a typo, correct 77704 to 77041

ObjID <- dfnew %>% filter(str_detect(zipcode,"77704")) # who did we change?
foo <-  df[df$siteaddid %in% ObjID$siteaddid,] %>% 
  mutate(change="Corrected zipcode from 77704 to 77041") 
corrections <- bind_rows(corrections, foo)

dfnew <- dfnew %>% 
  mutate(zipcode=str_replace(zipcode, "77704", "77041"))

```

zipcodes look okay, with one small exception.

```{r sources}

dfnew$source %>% unique() %>% sort()
...
```{r convert to googlecrs}

#   accept new roadnames

dfnew <- dfnew %>% 
  mutate(roadname=New_street)

#   Convert lat-longs to googlecrs

dfnew <- sf::st_transform(dfnew, crs=googlecrs)
...
# dfnew <- readRDS(paste0(path, "temporary_intermediate_file_4"))
```
...
df_nodups <- 
# dfnew %>% #head(50000) %>% 
dfnew %>% sf::st_drop_geometry() %>% # We'll add back later
  group_by(zipcode, roadname, addrnum, roadtype, prefix) %>% 
  summarise(n=n(), 
            addrnum=last(addrnum),
            prefix=last(prefix),
            roadname=last(roadname),
            roadtype=last(roadtype),
            ...
            # Lat=mean(Y),
            # Lon=mean(X),
            Xmax=(max(xcoord)-min(xcoord)),
            Ymax=(max(ycoord)-min(ycoord)),
  ...
  select(addrnum, prefix, roadname, roadtype, zipcode,
         # n, Diagonal, Xmax, Ymax, Xstd, Ystd, X, Y, Lat, Lon) %>% 
         n, Diagonal, Xmax, Ymax, Xstd, Ystd, X, Y) %>% 
  filter(n>1)
...
#   First pull out appropriate records from df_nodups and combine with dfnew

tmp <- df_nodups %>% filter(Diagonal>500) %>% # let's start with the worst ones
  select(addrnum, prefix, roadname, roadtype, zipcode, Diagonal, n) %>% 
  inner_join(dfnew,by=c("addrnum", "prefix", "roadname", 
                        "roadtype", "zipcode")) %>% 
  sf::st_as_sf()
...
a <- sf::st_intersects(tmp, temp, sparse = TRUE)
...
for (i in 1:nrow(Bad_zips)){
  print(paste("---", i, "---", Bad_zips[i,]$roadname))
  Bad_zips[i,]$hits <- 
    sum(str_detect(dfnew$zipcode,Bad_zips[i,]$Zip) & 
        str_detect(dfnew$roadname,Bad_zips[i,]$roadname)
  )
#   How close to bad zipcode is the point? Boundary of zip may have moved.
  Bad_zips[i,]$closeness <- as.numeric(sf::st_distance(Bad_zips[i,], 
...
ObjID <- Bad_zips[Bad_zips$hits<3,]$siteaddid

foo <-  df[df$siteaddid %in% ObjID,] %>% 
...
for (i in 1:nrow(Bad_zips)){
  if (Bad_zips[i,]$hits<3) {
    dfnew <- dfnew[dfnew$siteaddid!=Bad_zips[i,]$siteaddid,]
  }
}
  
#   for hits >=3, correct the zipcode
ObjID <- Bad_zips[Bad_zips$hits>=3,]$siteaddid

foo <-  df[df$siteaddid %in% ObjID,] %>% 
  mutate(change=paste("Location mismatches zipcode, correct it to", 
                       Bad_zips[Bad_zips$siteaddid %in% ObjID,]$Zip)) 

foo2 <- Bad_zips[Bad_zips$siteaddid %in% ObjID,] %>% 
  select(siteaddid, Zip)
df_nosf <- df %>% sf::st_drop_geometry()
foo <- left_join(df_nosf[df_nosf$siteaddid %in% ObjID,],
                 foo2,
                 by="siteaddid") %>% 
  mutate(change=paste("Location mismatches zipcode, correct it to", Zip )) %>% 
  select(-Zip)

corrections <- bind_rows(corrections, sf::st_as_sf(foo, crs=COH_crs))

Bad_zips <- Bad_zips %>% filter(hits>=3)
for (i in 1:nrow(Bad_zips)){
    dfnew[dfnew$siteaddid==Bad_zips[i,]$siteaddid,]$zipcode <- Bad_zips[i,]$Zip 
}
...
df_prefix <- 
  dfnew %>% sf::st_drop_geometry() %>% # This will speed things considerably
  group_by(zipcode, roadname, roadtype) %>% 
    summarize(n=n(),
              sumNA = sum(prefix==""),
              sumE = sum(prefix=="E", na.rm=TRUE),
              sumW = sum(prefix=="W", na.rm=TRUE),
...
df_bad <- inner_join(dfnew, bad_pts, 
                     by=c("zipcode", "roadname", "roadtype")) %>% 
  sf::st_drop_geometry()

Save_stats <- NULL
df_worse <- df_bad[df_bad$prefix=="",]
####df_worse <- df_worse %>% filter(Street_name=="PEACEFUL CANYON")
#   For each blank prefix look for the nearest companion with a prefix
for (i in 1:nrow(df_worse)){
  pt <- df_worse[i,]
  print(paste("----", pt$addrnum, pt$roadname, pt$roadtype))
  eraseme <-  df_bad %>% 
    filter(roadname==pt$roadname,
           roadtype==pt$roadtype) %>% 
    mutate(eucdist=sqrt((xcoord - pt$xcoord)^2 +
                          (ycoord - pt$ycoord)^2)) %>% 
    filter(eucdist <300,
           !prefix=="") %>% 
...
Save_stats <- Save_stats %>% 
  filter(num_pts >= 5) %>% 
  select(siteaddid, prefix_new=prefix)

#   Update corrections and dfnew

ObjID <- Save_stats$siteaddid
foo <- left_join(df[df$siteaddid %in% ObjID,],
                 Save_stats,
                 by="siteaddid") %>% 
...
dfnew[dfnew$siteaddid %in% ObjID,]$prefix <- Save_stats$prefix_new
...
Save_stats <- NULL
df_worse <- df_bad[df_bad$prefix=="",]
####df_worse <- df_worse %>% filter(roadname=="PEACEFUL CANYON")
for (i in 1:nrow(df_worse)){
  pt <- df_worse[i,]
  print(paste("----", pt$addrnum, pt$roadname, pt$roadtype))
  eraseme <-  df_bad %>% 
...
eraseme <- 
  dfnew %>% sf::st_drop_geometry() %>% # This will speed things considerably
  mutate(pseudotype=str_extract(roadname, "\\w+$")) %>% 
  filter(pseudotype==roadtype) %>% 
  group_by(roadname) %>% 
    summarize(roadtype=last(roadtype),
              n=n())
...
Old_street <- c("MARTIN LUTHER KING$", "BARON BROOK DR", "BROKEN BACK DR STE",
                ...
                "COUNTRY VILLAGE BLVD", "CREEK CIR", 
                "HOBBY AIRPORT LOOP", "MOSSY OAK DR", "MOUNTAIN MAPLE CT",
                "OLD MAIN LOOP", "PETINA CYPRESS CT", "OLIVER COTTON WAY")
...
New_street <- c("MARTIN LUTHER KING JR", "BARON BROOK", "BROKEN BACK",
                ...
                "COUNTRY VILLAGE", "CREEK", 
                "HOBBY AIRPORT", "MOSSY OAK", "MOUNTAIN MAPLE",
                "OLD MAIN", "PETINA CYPRESS", "OLIVER COTTON")

#  First repair type where needed

dfnew[str_detect(dfnew$roadname, "FAIRWAY DR STE"),]$roadtype <- "DR"
dfnew[str_detect(dfnew$roadname, "PAXTON DRIVR"),]$roadtype <- "DR"
dfnew[str_detect(dfnew$roadname, "HAVEN CREEK DR/BACKS"),]$roadtype <- "DR"
dfnew[str_detect(dfnew$roadname, "BLACK FALCON RD"),]$roadtype <- "RD"
dfnew[str_detect(dfnew$roadname, "LA PORTE FWY"),]$roadtype <- "FWY"
dfnew[str_detect(dfnew$roadname, "FRANK SCOTT BLVD"),]$roadtype <- "BLVD"

#   Now repair names

for (i in 1:length(Old_street)) {
  print(Old_street[i])
  dfnew[str_detect(dfnew$roadname,Old_street[i]),]$roadname <- New_street[i]
}

ObjID <- dfnew %>% filter(str_detect(New_street, 
                                  paste(Old_street, collapse="|")))  

foobar <-  df[df$siteaddid %in% ObjID$siteaddid,] %>% 
  mutate(change=paste("Corrected street name to", 
                       dfnew[dfnew$siteaddid %in% ObjID$siteaddid,]$roadname)) 
...
dfnew %>% 
  ...
  summarise(n=n(),
            ...
            xcoord=mean(xcoord),
            ycoord=mean(ycoord),
            siteaddid=last(siteaddid)
            ) %>% 
  select(addrnum, prefix, roadname, roadtype, municipality, zipcode,
         n, Lat, Lon, Xstd, Ystd, X=xcoord, xmed, Y=ycoord, ymed, siteaddid)  

dfnew2 <- dfnew2 %>% 
  mutate(DeltaX=abs(X-xmed), DeltaY=abs(Y-ymed))

#   Final cleanup. Clusters of identical addresses that have too large a spread
#   in location, based on difference between median and mean locations, I will
#   just delete if there are fewer than 5 points and the deltaX or 
#   deltaY are greater than 150.

dfnew2 <- dfnew2 %>% 
  filter(!(n<5 & DeltaX>150)) %>% 
  filter(!(n<5 & DeltaY>150))  

#   File suitable for using in geocoding

dfnew <- dfnew2 %>% 
  select(Street_num=addrnum, Prefix=prefix, Street_name=roadname, 
         Street_type=roadtype, City=municipality, Zipcode=zipcode,
         n, Lat, Lon, Xstd, Ystd, X=xmed, Y=ymed, OBJECTID=siteaddid)
