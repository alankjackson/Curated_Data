...
dfnew$addrnum <- as.character(dfnew$addrnum)
dfnew <- dfnew %>% replace_na(list(prefix="", roadtype=""))
...
dfnew %>% 
  #head(10) %>% 
  select(siteaddid, STREET_NAME, addrnum, zipcode, X, Y, xcoord, ycoord) %>% 
  #select(siteaddid, STREET_NAME, addrnum, zipcode, xcoord, ycoord)# %>% 
...
ObjID <- dfnew %>% 
  filter(ycoord<13000000) # pull out records with bad Y coordinate

foo[foo$siteaddid %in% ObjID$siteaddid,]

dfnew[dfnew$siteaddid %in% ObjID$siteaddid,]$ycoord <- 
  unname(foo[foo$siteaddid %in% ObjID$siteaddid,]$Y)

foo <-  df[df$siteaddid %in% ObjID$siteaddid,] %>% 
  mutate(change="Repaired ycoord by reprojecting") 
corrections <- bind_rows(corrections, foo)
...
dfnew %>% #head(20000) %>% 
  ...
  select(addrnum, prefix, Street_name, roadtype, zipcode,
...
tmp <- df_nodups %>% filter(Diagonal>500) %>% # let's start with the worst ones
  select(addrnum, prefix, Street_name, roadtype, zipcode, Diagonal, n) %>% 
  inner_join(dfnew,by=c("addrnum", "prefix", "Street_name", 
                        "roadtype", "zipcode"))
