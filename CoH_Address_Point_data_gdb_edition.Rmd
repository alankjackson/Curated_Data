...
df_prefix <- dfnew %>% 
  group_by(zipcode, Street_name, roadtype) %>% 
    summarize(n=n(),
              sumNA = sum(prefix==""),
              sumE = sum(prefix=="E", na.rm=TRUE),
              sumW = sum(prefix=="W", na.rm=TRUE),
...
df_bad <- inner_join(dfnew, bad_pts, 
                     by=c("zipcode", "Street_name", "roadtype"))
...
for (i in 1:nrow(df_worse)){
  pt <- df_worse[i,]
  print(paste("----", pt$addrnum, pt$Street_name, pt$roadtype))
  eraseme <-  df_bad %>% 
...
#####Save_stats[duplicated(Save_stats$siteaddid),]
...
dfnew[str_detect(dfnew$Street_name, "FAIRWAY DR STE"),]$roadtype <- "DR"
dfnew[str_detect(dfnew$Street_name, "PAXTON DRIVR"),]$roadtype <- "DR"
dfnew[str_detect(dfnew$Street_name, "HAVEN CREEK DR/BACKS"),]$roadtype <- "DR"
dfnew[str_detect(dfnew$Street_name, "BLACK FALCON RD"),]$roadtype <- "RD"
dfnew[str_detect(dfnew$Street_name, "LA PORTE FWY"),]$roadtype <- "FWY"
dfnew[str_detect(dfnew$Street_name, "FRANK SCOTT BLVD"),]$roadtype <- "BLVD"
...
foobar <-  df[df$siteaddid %in% ObjID$siteaddid,] %>% 
  mutate(change=paste("Corrected street name to", 
                       dfnew[dfnew$siteaddid %in% ObjID$siteaddid,]$Street_name)) 
...
dfnew <- 
dfnew %>% 
  group_by(zipcode, Street_name, addrnum, roadtype, prefix) %>% 
  summarise(n=n(),
            addrnum=last(addrnum),
            prefix=last(prefix),
            Street_name=last(Street_name),
  ...
  select(addrnum, prefix, Street_name, roadtype, City, zipcode,
