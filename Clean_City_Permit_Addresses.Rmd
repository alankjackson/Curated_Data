...
# 
 ...
 More_types <- paste(c(" MALL", "PLACE", "WAY"), collapse="| ")
 foo <- foo %>% 
   string_replace("Output", paste0("( ",More_types,") \\d+$"), "\\1", "Apply")  
...
All_types <- paste0(All_types, compact="|")

mask <- str_detect(df_names_only$Street_name, All_types) 
...
#for (i in 2500:5000){ # first look for exact matches takes about 1 minute
  ...
  if (tmp$Success){ #   success
    Exact_match <- cbind(foo[i,], tmp) %>% 
      select(pm.house, pm.preDir, pm.street, pm.streetSuf, Zipcode, Lat, Lon) %>% 
      rbind(., Exact_match)
  } else { #  Fail exact match
    Failed_match <- cbind(foo[i,], tmp) %>% 
      select(pm.house, pm.preDir, pm.street, pm.streetSuf, Zipcode, Fail,
             Lat, Lon) %>% 
      rbind(., Failed_match)
  }
}
...
for (i in 1:nrow(Failed_match)){
  if (Failed_match[i,]$Fail!="Street_type") {next} # skip if name isn't the issue
  if (i%%10==0) { print(paste("---",i,"---")) }
  target <- Failed_match[i,]
