...
Poly[1,]$Poly <- list(matrix(c(Poly[1,]$lrx, Poly[1,]$lry,
...
for (j in 2:84){ # left column
  Dx <- abs(Poly[j,]$X - Poly[ij(2,j+1),]$X)
  Dy <- abs(Poly[j,]$Y - Poly[ij(2,j+1),]$Y)
  print(paste(Dx, ",", Dy))
  Poly[j,]$llx <- Poly[j,]$X - Dx/2
  Poly[j,]$lly <- Poly[j,]$Y - Dy/2
  Poly[j,]$lrx <- Poly[j,]$X + Dx/2
  Poly[j,]$lry <- Poly[j,]$Y - Dy/2
  Poly[j,]$urx <- Poly[j-1,]$lrx
  Poly[j,]$ury <- Poly[j-1,]$lry 
  Poly[j,]$ulx <- Poly[j-1,]$llx 
  Poly[j,]$uly <- Poly[j-1,]$lly 
  #Poly[j,]$urx <- Poly[j,]$X + Dx/2
  #Poly[j,]$ury <- Poly[j,]$Y + Dy/2
  #Poly[j,]$ulx <- Poly[j,]$X - Dx/2
  #Poly[j,]$uly <- Poly[j,]$Y + Dy/2
  Poly[j,]$Poly <- list(matrix(c(Poly[j,]$lrx, Poly[j,]$lry,
                          Poly[j,]$llx, Poly[j,]$lly,
                          Poly[j,]$ulx, Poly[j,]$uly,
                          Poly[j,]$urx, Poly[j,]$ury,
                          Poly[j,]$lrx, Poly[j,]$lry),
                          ncol=2, nrow=5, byrow=TRUE))
  
}

Poly %>% head() %>% select(!c(Num, Letter, Poly, Key)) %>% 
    pivot_longer(
    #everything(),
