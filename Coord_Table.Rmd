...
for (i in 91:97) {
  for (j in 1:85){ # left column
    if ((i==1) & (j==1)) {next}
    print (paste(i, j))
    if (j<85) { # to catch the bottom row by using the last Dx, Dy
      Dx <- abs(Poly[ij(i,j),]$X - Poly[ij(i+1,j+1),]$X)
      Dy <- abs(Poly[ij(i,j),]$Y - Poly[ij(i+1,j+1),]$Y)
    }
    if (i==97) { # to catch the rightmost column
      if (j<85 & j>1) {
    ...
    #Poly[ij(i,j),]$urx <- Poly[ij(i,j),]$X + Dx/2
    #Poly[ij(i,j),]$ury <- Poly[ij(i,j),]$Y + Dy/2
    #Poly[ij(i,j),]$ulx <- Poly[ij(i,j),]$X - Dx/2
    #Poly[ij(i,j),]$uly <- Poly[ij(i,j),]$Y + Dy/2
    Poly[ij(i,j),]$Poly <- list(matrix(c(Poly[ij(i,j),]$lrx, Poly[ij(i,j),]$lry,
