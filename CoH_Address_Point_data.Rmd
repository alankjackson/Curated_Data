...
Old_street <- c("H M C", "^F M ", "^F.M. ",
                "^JOHN A$" ,"^AVE " ,"^H R$" ,"^T K C$")
New_street <- c("H MARK CROSSWELL JR", "FM ", "FM ", 
                "JOHN-A", "AVENUE ", "H AND R", "TKC")
...
#   Make an sf object
df_bad <- sf::st_as_sf(df_bad, coords=c("X", "Y"), crs=googlecrs, agr = "identity")

#######   neeed to pull out street in zip to calc distances to
for (pt in df_bad[df_bad$Prefix=="",]){
  print("----")
  foobar <- df_bad %>%  sf::st_distance(pt)
...
########   averaging process. Update df accordingly and there will be no 
########   duplicates left
...
dfnew %>% 
  ...
  select(Street_num, Prefix, Street_name, Street_type, City, Zipcode,
         n, Lat, Lon, Xstd, Ystd, X, Y, OBJECTID)  
...
dfnew <- dfnew %>% select(Street_num, Street_alpha, Prefix, Street_name, 
                          Street_type, City=CITY, Zipcode=ZIPCODE,
                          Lat=Y, Lon=X, X=X_COORD, Y=Y_COORD, OBJECTID
                          )
