...
B_bot <- (coords[2,5] - M_top*coords[2,4])[[1]]
...
Key_2_LL <- function(Num, Alpha) {
  Alpha_addY <- case_when(
                Alpha %in% c("A", "B", "C", "D") ~ 0,
                Alpha %in% c("E", "F", "G", "H") ~ 1,
                Alpha %in% c("J", "K", "L", "M") ~ 2,
                Alpha %in% c("N", "P", "Q", "R") ~ 3,
                Alpha %in% c("S", "T", "U", "V") ~ 4,
                Alpha %in% c("W", "X", "Y", "Z") ~ 5
              )
  ...
  if (Num < 715) {
    ...
    Y <- 67 + Alpha_addY #  Y index 
  } else if (Num > 748) {
    X <- ((Num-749)%%34)*4 + 1 + Alpha_addX #  X index 
    Y <- ((Num-((Num+1-749)%%34)-748)/34)*6 + 73 + Alpha_addY #  Y index 
  }
